...
- ⚙️ **自动化沉淀**：`promote.sh` 一键将通过测试的 commit 从 dev cherry-pick 到 main
- 🛡️ **安全回滚**：`rollback.sh` 一键将 dev 重置到 staging，及时止损
...
选一个目标 → 先写测试 → dev 上实现 → 跑测试
  ├─ 全部通过 → 执行代码自审查 → 通过则运行 promote.sh 沉淀到 main
...
│   └── GOALS.md                # 目标追踪模板
├── scripts/
│   ├── promote.sh              # 沉淀脚本（dev → main）
│   └── rollback.sh             # 回滚脚本（dev → staging）
└── .git/hooks/
